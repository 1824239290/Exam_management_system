...
<!-- endinject -->
<script>
    var  tpid=window.location.search;
    tpid=tpid.replace(/[^\d]/g,"");
    function detail() {
        axios({
            url:"/testPaper/"+tpid,
            method:"get"
            ...
            $.each(data1,function (k,v){
                var type=data1[k].type;
                type=type==="schose"?"单选题":(type==="multiple"?"多选题":"判断题");
                html +=`<tbody>
                        <tr>
                          <td>${data1[k].name}</td>
                          <td>${data1[k].optionA}</td>
                          <td>${data1[k].optionB}</td>
                          <td>${data1[k].optionC}</td>
                          <td>${data1[k].optionD}</td>
                          <td>${data1[k].answer}</td>
                          <td>${data1[k].analysis}</td>
                          <td>${type}</td>
                        </tr>
                      </tbody>`
            });
