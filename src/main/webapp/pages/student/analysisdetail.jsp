...
<script>
            ...
            $.each(data1,function (k,v){
                html +=`<p class="exam-title exam-schose">${k+1}、${data1[k].tname}</p>
                            ...
                            <input type="hidden" value="${data1[k].tid}" name="stid${k+1}"/>
                                <div class="p-div-input">A、${data1[k].optionA}</div>
                                <div class="p-div-input">B、${data1[k].optionB}</div>
                                <div class="p-div-input">C、${data1[k].optionC}</div>
                                <div class="p-div-input">D、${data1[k].optionD}</div>
                                <div class="sanswer schoseSelected${k+1}"></div>
                                <div class="analysis">正确答案:${data1[k].answer}，解析:${data1[k].analysis}</div>
                            </div>`
            });[]
            $("#schose").append(html)
        });
            ...
            $.each(data1,function (k,v){
                html +=`<p class="exam-title exam-multiple">${k+1}、${data1[k].mname}</p>
                            ...
                            <input type="hidden" value="${data1[k].mid}" name="mtid${k+1}"/>
                                ...
                                <div class="p-div-input">C、${data1[k].optionC}</div>
                                <div class="p-div-input">D、${data1[k].optionD}</div>
                                <div class="sanswer multipleSelected${k+1}"></div>
                                <div class="analysis">正确答案:${data1[k].answer}，解析:${data1[k].analysis}</div>
                            </div>`
            });[]
            ...
            $.each(data1,function (k,v){
                html +=`<p class="exam-title exam-judgment">${k+1}、${data1[k].jname}</p>
                            ...
                            <input type="hidden" value="${data1[k].jid}" name="jtid${k+1}"/>
                                <div class="p-div-input">A、${data1[k].optionA}</div>
                                <div class="p-div-input">B、${data1[k].optionB}</div>
                                <div class="sanswer judgmentSelected${k+1}"></div>
                                <div class="analysis">正确答案:${data1[k].answer}，解析:${data1[k].analysis}</div>
                            </div>`
            });[]
    ...
    //获取学生的单选题答案
    function getSchoseSelected() {
        axios({
            url:`/reply/getselected?pid=${pid}&uid=${uid}&type=schose`,
            method:"get"
        }).then(function (res) {
            data1=res.data.data;
            $.each(data1,function (k,v) {
                var sel=".schoseSelected"+(k+1);
                var ans="我的答案："+data1[k].selected+"&nbsp;&nbsp;";
                $(sel).append(ans);
            })
    ...
    function getMultipleSelected() {
        axios({
            url:`/reply/getselected?pid=${pid}&uid=${uid}&type=multiple`,
            method:"get"
        }).then(function (res) {
            data1=res.data.data;
            $.each(data1,function (k,v) {
                var sel=".multipleSelected"+(k+1);
                var ans="我的答案："+data1[k].selected+"&nbsp;&nbsp;";
                $(sel).append(ans);
            })
        })
    ...
    //获取学生的判断题答案
    function getJudgmentSelected() {
        axios({
            url:`/reply/getselected?pid=${pid}&uid=${uid}&type=judgment`,
            method:"get"
        }).then(function (res) {
            data1=res.data.data;
            $.each(data1,function (k,v) {
                var sel=".judgmentSelected"+(k+1);
                var ans="我的答案："+data1[k].selected+"&nbsp;&nbsp;";
                $(sel).append(ans);
            })
        })
